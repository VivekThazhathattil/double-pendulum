...
void resetTextFields(sf::Text &L1, sf::Text &L2, sf::Text &m1, sf::Text &m2,
                     pendulum *a1, pendulum *a2) {
...
void changeShape(sf::RectangleShape &rod1, sf::RectangleShape &rod2,
                 sf::CircleShape &bob1, sf::CircleShape &bob2, pendulum *a1,
                 pendulum *a2) {
...
void ang_acc(float &m1, float &m2, float &g, float &a1, float &a2, float &a1_v,
             float &a2_v, float &L1, float &L2, float *res) {
...
int main() {
  ...
  bannerMsg.setPosition(
      window.getSize().x / 2 - bannerMsg.getLocalBounds().width / 2, 0);
  ...
  while (window.isOpen()) {
    ...
    if (!isPaused) {
      ...
      bob1.setPosition(p1.o[0] + p1.r * cos(M_PI / 2 + p1.a) - p1.m,
                       p1.o[1] + p1.r * sin(M_PI / 2 + p1.a) - p1.m);
      ...
      bob2.setPosition(p2.o[0] + p2.r * cos(M_PI / 2 + p2.a) - p2.m,
                       p2.o[1] + p2.r * sin(M_PI / 2 + p2.a) - p2.m);
      if (streak == 's') {

        streakLines[streak_count % STREAKNUM].position =
            sf::Vector2f(p2.o[0] + p2.r * cos(M_PI / 2 + p2.a) - p2.m,
                         p2.o[1] + p2.r * sin(M_PI / 2 + p2.a) - p2.m);
        streak_count =
            streak_count == STREAKNUM * 10 - 1 ? 0 : streak_count + 1;
      }
    }
